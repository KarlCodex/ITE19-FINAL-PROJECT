...
// Function prototypes
void removeSpaces(char *str); // Function to remove spaces from a string
void calculate(char *line, char *outputBuffer); // Function to calculate expression result
void readInputFile(char symbols[50], char data[100], int *symbolIndex, char *outputBuffer); // Function to read input file
void convertNumberToWords(int num, char *output); // Function to convert number to words
void saveToFile(char *output); // Function to save result to file
void displayFromFile(); // Function to display result from file

// Function to read the input file and process each line
void readInputFile(char symbols[50], char data[100], int *symbolIndex, char *outputBuffer) {
    ...
    while (fgets(data, 100, filePointer)) { // Read each line from the file
        ...
        removeSpaces(data); // Remove trailing spaces from the line
...
int convertRoman(char *roman) {
    ...
    while (roman[i] != '\0') { // Loop through each character in the Roman numeral string
        int current = romanToInteger(roman[i]); // Get the integer value of the current character
        int next = romanToInteger(roman[i + 1]); // Get the integer value of the next character
...
void calculate(char *line, char *outputBuffer) {
    ...
    int num1 = convertRoman(firstNum); // Convert the first Roman numeral to integer
    int num2 = convertRoman(secondNum); // Convert the second Roman numeral to integer
    ...
    char resultWords[200]; // Buffer to store result in words
    convertNumberToWords(result, resultWords); // Convert the result to words
    strcat(outputBuffer, resultWords); // Append the result words to the output buffer
...
int main() {
    ...
    readInputFile(symbols, data, &symbolIndex, outputBuffer); // Read and process the input file
    saveToFile(outputBuffer); // Save the output to a file
    displayFromFile(); // Display the contents of the output file
